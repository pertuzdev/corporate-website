...
export const ProjectItemWrapper = styled(Tilt)`
  ...
  @media screen and (max-width: 768px) {
    ...
    max-width: 100%;
    flex-basis: 100%;
    padding: ${({ amount, index }) => isFirstInPairLayout(amount, index) ? 0 : '20px'};
  }
`
export const ProjectItemTextWrapper = styled.div`
  width: ${({ amount, index }) => isFirstInPairLayout(amount, index) && '45%'};
  padding: ${({ amount, index }) => isFirstInPairLayout(amount, index) && '20px'};
  border-top-left-radius: 15px;
  border-top-right-radius: 15px;
  @media screen and (max-width: 768px) {
    width: unset;
    background: ${({ amount, index }) => isFirstInPairLayout(amount, index) && '#3a3c70'};
    clip-path: ${({ amount, index }) => isFirstInPairLayout(amount, index) && 'polygon(0 0, 100% 0%, 100% 93%, 0 100%)'};
  }
`
